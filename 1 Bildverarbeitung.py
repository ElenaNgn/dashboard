...
with st.sidebar:
    ...
    copy_to_s_drive = False  # Default für Upload-Modus
    ...
    if processing_mode == "📁 Netzwerkpfad verwenden":
        ...
        # Kopieren-Option nur im Netzwerk-Modus anzeigen
        copy_to_s_drive = st.checkbox(
            "📋 Dateien ins S-Laufwerk kopieren",
            value=True,
            key="copy_to_s_drive_checkbox",
            help="Kopiert alle verarbeiteten Dateien automatisch ins S:/Multimedia Laufwerk"
        )
...
with tab1:
    if processing_mode == "📤 Bilder hochladen":
        ...
        if st.button("🚀 Verarbeitung starten", type="primary", disabled=not uploaded_files):
            results = process_images_streamlit(uploaded_files, copy_to_s_drive=False)  # Im Upload-Modus immer False
...
with tab2:
    ...
    **Netzwerk-Modus (Standard):**
    ...
    2. Optional: S-Laufwerk-Kopie aktivieren/deaktivieren
    3. Verarbeitung starten
    4. Dateien im Netzwerk gespeichert
    ...
    **Upload-Modus:**
    ...
    - JPEG (Katalog)
    - XLSX (Import-Datei)
    - Optional: Kopie auf S-Laufwerk (nur Netzwerk-Modus)
    """)
